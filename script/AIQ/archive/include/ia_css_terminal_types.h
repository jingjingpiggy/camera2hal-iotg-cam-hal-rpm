...
struct ia_css_param_payload_s {
	uint64_t			host_buffer;						/**< Temporary variable holding the host address of the parameter buffer as PSYS
													is handling the parameters on the host side for the moment */
	vied_vaddress_t			buffer;							/**< Base virtual addresses to parameters in subsystem virtual memory space */
	uint8_t				padding[N_PADDING_UINT8_IN_PARAM_PAYLOAD_STRUCT];	/**< vied_vaddress_t == 32 bits, align to 64 */
};
...
struct ia_css_param_section_desc_s {				/**< Frame constant parameters section */
...
struct ia_css_param_terminal_s {					/**< Frame constant parameters terminal */
	ia_css_terminal_t		base;				/**< Parameter terminal base */
	ia_css_param_payload_t		param_payload;			/**< Parameter buffer handle attached to the terminal */
	uint16_t			param_section_desc_offset;	/**< Points to the variable array of ia_css_param_section_desc_t */
...
struct ia_css_fragment_grid_desc_s {
	uint16_t			fragment_grid_index[IA_CSS_N_DATA_DIMENSION];		/**< Offset width/height of the top-left compute unit of the fragment compared to the frame */
	uint16_t			fragment_grid_dimension[IA_CSS_N_DATA_DIMENSION];	/**< Resolution width/height of the spatial parameters that correspond to the fragment measured in compute units */
};
...
struct ia_css_frame_grid_param_section_desc_s {			/**< A plane of parameters with spatial aspect (compute units correlated to pixel data) */
	uint32_t			mem_offset;		/**< Offset of the parameter allocation in memory */
	uint32_t			mem_size;		/**< Memory allocation size needs of this parameter */
...
struct ia_css_frame_grid_desc_s {
	uint16_t			frame_grid_dimension[IA_CSS_N_DATA_DIMENSION];	/**< Resolution width/height of the frame of spatial parameters measured in compute units */
...
struct ia_css_spatial_param_terminal_s {
	ia_css_terminal_t		base;						/**< Spatial Parameter terminal base */
	ia_css_param_payload_t		param_payload;					/**< Spatial Parameter buffer handle attached to the terminal */
	ia_css_frame_grid_desc_t	frame_grid_desc;				/**< Contains info for the frame of spatial parameters */
	uint32_t			kernel_id;					/**< Kernel identifier */
	uint16_t			frame_grid_param_section_desc_offset;		/**< Points to the variable array of ia_css_frame_grid_param_section_desc_t */
	uint16_t			fragment_grid_desc_offset;			/**< Points to array of ia_css_fragment_spatial_desc_t which constain info for the fragments of spatial parameters */
};
...
struct ia_css_slice_param_section_desc_s {			/**< A Slice of parameters ready to be trasferred from/to registers */
	uint32_t			mem_offset;		/**< Offset of the parameter allocation in memory */
	uint32_t			mem_size;		/**< Memory allocation size needs of this parameter */
};
...
struct ia_css_fragment_slice_desc_s {
	uint16_t			slice_section_desc_offset;	/**< Points to array of ia_css_slice_param_section_desc_t which constain info for each prameter slice */
...
struct ia_css_sliced_param_terminal_s {
	...
	ia_css_param_payload_t		param_payload;					/**< Spatial Parameter buffer handle attached to the terminal */
	uint32_t			kernel_id;					/**< Kernel identifier */
	uint16_t			fragment_slice_desc_offset;			/**< Points to array of ia_css_fragment_slice_desc_t which constain info for the slicing of the parameters */
...
struct ia_css_fragment_param_section_desc_s {			/**< Fragment constant parameters section */
	uint32_t			mem_offset;		/**< Offset of the parameter allocation in memory */
	uint32_t			mem_size;		/**< Memory allocation size needs of this parameter */
};
...
struct ia_css_kernel_fragment_sequencer_command_desc_s {	/**< 4 commands packe together to save memory space */
	uint16_t			line_count[IA_CSS_N_COMMAND_COUNT];		/**< Contains the "(command_index%4) == index" command desc */
};
...
struct ia_css_kernel_fragment_sequencer_info_desc_s {
	...
	int16_t				fragment_grid_overlay_on_fragment_pixel_topleft_index[IA_CSS_N_DATA_DIMENSION];		/**< Relative position of grid origin to pixel origin */
	int16_t				fragment_grid_overlay_on_fragment_pixel_dimension[IA_CSS_N_DATA_DIMENSION];		/**< Size of active fragment region */
	uint16_t			command_count;										/**< If >0 it overrides the standard fragment sequencer info */
	uint16_t			command_desc_offset;									/**< To be used only if command_count>0, points to the descriptors for the commands (ia_css_kernel_fragment_sequencer_command_desc_s) */
};
